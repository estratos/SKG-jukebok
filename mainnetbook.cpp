...
MainNetbook::MainNetbook(QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::MainNetbook)
{
    ...
    ui->frameSelectedAlbum->setVisible(false);
    ui->nextPanel->setVisible(false);
    ui->prevPanel->setVisible(false);
...
//    ui->currPanel->LoadAlbums(Albums.at(0),Albums.at(1));
    ...
    EffectsObject->setCurrentSource(EffectsSource);MusicObject->setTickInterval(1000);
...
MainNetbook::~MainNetbook()
{
    delete ui;
}

void MainNetbook::changeEvent(QEvent *e)
{
...
void MainNetbook::SetConfig(JukeboxConfig *config)
{
    Config = config;
    ReloadConfig();
...
void MainNetbook::mousePressEvent(QMouseEvent *event)
{
    QRect *rect;
...
void MainNetbook::resizeEvent(QResizeEvent *event)
{
...
void MainNetbook::keyPressEvent(QKeyEvent *keypress)
{
    QString Custom;
...
void MainNetbook::DisplayCurrTrack()
{
    int albumnum;
    SKG_Album *curralbum;
    QTime *tracktime;
    ...
    //curralbum = Library->GetAlbumByNumber(albumnum);
...
//void MainNetbook::DisplayTrackNumbers()
//{
...
void MainNetbook::LetterButtonClicked(QString btnCaption)
{
    int tmpindex = 0;
    QChar *artistname;
    ...
    if (lessthancurrent)
    {
        ...
        //ui->prevPanel->stackUnder(ui->currPanel);
        ui->prevPanel->raise();
        ui->prevPanel->LoadAlbums(ui->currPanel->Album1(),ui->currPanel->Album2(),LargeAlbumNumbers());

        ui->currPanel->Clear();
        if ((PanelIndex + 1) < Albums.count())
            ui->currPanel->LoadAlbums(Albums.at(PanelIndex),Albums.at(PanelIndex + 1),LargeAlbumNumbers());
        else
            ui->currPanel->LoadAlbums(Albums.at(PanelIndex),NULL,LargeAlbumNumbers());
    ...
    else
    {
        ...
        ui->nextPanel->setGeometry(this->width() * 2,
                                   ...
                                   ui->currPanel->height());
        if ((PanelIndex + 1) < Albums.count())
            ui->nextPanel->LoadAlbums(Albums.at(PanelIndex),Albums.at(PanelIndex + 1),LargeAlbumNumbers());
        else
            ui->nextPanel->LoadAlbums(Albums.at(PanelIndex),NULL,LargeAlbumNumbers());
...
void MainNetbook::ButtonClicked(QString btnCaption)
{
    int albumnum;
    int tracknum;
            ...
            if (TrackString.length() == 3)
            {
                ...
                else
                {
                    if (TrackString.toInt() <= Albums.count())
                    {
                        ...
                        //SelectedAlbum = Library->GetAlbumByNumber(albumnum);
                        ...
                        ui->currPanel->Clear();
                        //ui->currPanel->LoadAlbums(Library->GetAlbumByNumber(PanelIndex),
                        //                          Library->GetAlbumByNumber(PanelIndex + 1));
                        ui->currPanel->LoadAlbums(Albums.at(PanelIndex - 1),Albums.at(PanelIndex));
                    }
                    else
                    {
                        TrackString = "";
...
void MainNetbook::PrevButtonClicked()
{
    DisplayPreviousPanel();
}

void MainNetbook::NextButtonClicked()
{
    DisplayNextPanel();
}

void MainNetbook::DisplayConfigForm()
{
...
void MainNetbook::DisplayOptionsForm()
{
    ...
    if (OptionForm.exec() == QDialog::Accepted)
    {
        ...
        else if (OptionForm.Option == OptionRescan)
        {
...
void MainNetbook::DisplayTrackListForm()
{
    int res;
...
void MainNetbook::DisplayNextPanel()
{
    if (!SlideFinished)
        return;
...
//    QPropertyAnimation animation(ui->nextPanel,"geometry");
//    animation.setDuration(500);
//    animation.setStartValue(QRect(this->x(), ui->currPanel->y(), ui->currPanel->width(), ui->currPanel->height()));
//    animation.setEndValue(QRect(ui->currPanel->x(), ui->currPanel->y(), ui->currPanel->width(), ui->currPanel->height()));
//    animation.start();
//    SlideFinished = true;
}

void MainNetbook::DisplayPreviousPanel()
{
    if (!SlideFinished)
        return;
...
void MainNetbook::SlideNextPanelLeft()
{
    //if ((ui->nextPanel->x() - MoveIncrement) <= 0)
    if ((ui->nextPanel->x()) <= 0)
    {
        timerSlideLeft->stop();
        ui->currPanel->Clear();
...
void MainNetbook::SlideCurrentPanelRight()
{
...
void MainNetbook::Album1Clicked()
{
    SelectedAlbum = ui->currPanel->Album1();
    DisplaySelectedAlbum();
}

void MainNetbook::Album2Clicked()
{
    SelectedAlbum = ui->currPanel->Album2();
    DisplaySelectedAlbum();
}

void MainNetbook::AddSelectedTrack()
{
    long newtracknum;
    ...
    if (newtracknum % 100 == 0)
    {
            ...
            (MusicObject->currentSource().type() == Phonon::MediaSource::Invalid))
        {
            ...
            currTrack = TrackByNumber(firsttracknum);
...
void MainNetbook::CancelSelectedTrack()
{
    SelectedAlbum = NULL;
    TrackString = "";
    ui->lblTrackString->setText("Track List");
...
void MainNetbook::DisplaySelectedAlbum()
{
...
void MainNetbook::Btn_0Clicked()
{
    ButtonClicked("0");
}

void MainNetbook::Btn_1Clicked()
{
    ButtonClicked("1");
}

void MainNetbook::Btn_2Clicked()
{
    ButtonClicked("2");
}

void MainNetbook::Btn_3Clicked()
{
    ButtonClicked("3");
}

void MainNetbook::Btn_4Clicked()
{
    ButtonClicked("4");
}

void MainNetbook::Btn_5Clicked()
{
    ButtonClicked("5");
}

void MainNetbook::Btn_6Clicked()
{
    ButtonClicked("6");
}

void MainNetbook::Btn_7Clicked()
{
    ButtonClicked("7");
}

void MainNetbook::Btn_8Clicked()
{
    ButtonClicked("8");
}

void MainNetbook::Btn_9Clicked()
{
    ButtonClicked("9");
}

void MainNetbook::SelectedTrackChanged()
{
    TrackString = ui->SelectedAlbumWidget->SelectedTrackID();
    ui->lblTrackString->setText(TrackString);
}

void MainNetbook::QueueNextTrack()
{
    if (TrackNumbers.count() > 0)
    {
...
void MainNetbook::tick(qint64 time)
{
...
void MainNetbook::DisplayAlbumArtForm()
{
...
void MainNetbook::DisplayAlbumArtMultiForm()
{
    if (!Config->IsConnected())
    {
        QMessageBox box;
...
void MainNetbook::AlbumArtClicked()
{
    DisplayAlbumArtForm();
    ui->SelectedAlbumWidget->Clear();
    ui->SelectedAlbumWidget->LoadAlbum(SelectedAlbum,LargeAlbumNumbers());
...
void MainNetbook::LoadCustomAlbumList(CustomAlbumList *list)
{
    int i;
    SKG_Album *curralbum;
    ...
    ui->currPanel->Clear();
    ui->currPanel->LoadAlbums(Albums.at(0),Albums.at(1),LargeAlbumNumbers());
    PanelIndex = 0;
}

SKG_Track* MainNetbook::TrackByNumber(long tracknum)
{
    int albumnumber;
    int tracknumber;
...
void MainNetbook::DisplayLoadCustomListForm()
{
...
void MainNetbook::ReloadConfig()
{
    ...
    QFileInfo *libinfo = new QFileInfo(LibraryFile());
    if(libinfo->exists())
    {
...
void MainNetbook::DisplayRipForm()
{
    if (!Config->IsConnected())
    {
        QMessageBox box;
...
void MainNetbook::DisplayPlaylistForm()
{
...
void MainNetbook::DisplayHelpForm()
{
    frmHelp HelpForm;
    HelpForm.exec();
}

void MainNetbook::LoadPlaylist(Playlist *plist,bool random)
{
    QList<int> *tracknums;
    SKG_Track *currtack;
...
void MainNetbook::TranslateCustomKeystroke(QString Custom)
{
...
bool MainNetbook::LargeAlbumNumbers()
{
    return (Config->GetCollectionSize() == sizeLarge);
}

void MainNetbook::SaveTrackList()
{
...
void MainNetbook::LoadTrackList()
{
    //SKG_Track *currTrack;
    QFile f(TrackListFile());
    if (!f.exists())
...
void MainNetbook::DisplayTrackSearchForm()
{
...
void MainNetbook::AddTrackNumber(long tracknum)
{
    if ((MusicObject->currentSource().type() == Phonon::MediaSource::Empty) ||
        (MusicObject->currentSource().type() == Phonon::MediaSource::Invalid))
    {
...
void MainNetbook::AddCurrentTrackToPlaylist()
{
...
void MainNetbook::PlayRandomTrack()
{
    int randomalbum;
    int randomtrack;
    SKG_Album *album;
...
void MainNetbook::VolumeUp()
{
    MusicOutput->setVolumeDecibel(MusicOutput->volumeDecibel() + 2);
}

void MainNetbook::VolumeDown()
{
    MusicOutput->setVolumeDecibel(MusicOutput->volumeDecibel() - 2);
}

void MainNetbook::DisplayVolumeForm()
{
    if (MusicObject->state() != Phonon::PlayingState)
        return;
...
void MainNetbook::DisplayAboutForm()
{
    frmAbout abt;
    abt.exec();
}

void MainNetbook::NextLetter()
{
    QString currletter;
